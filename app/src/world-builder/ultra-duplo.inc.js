 ...
 * world-builder.html injiziert und nutzt deshalb dessen interne Editor-Helfer.
 * Ziel: Das Messegelaende zuerst als wenige grobe, gut greifbare Baukloetze
 * bearbeiten; jeder Klotz kann in verknuepfte Bauplan-Details oder spaeter in
 * eine GLB aufklappen und danach verlustfrei wieder zum Klotz werden.
 */
...
function ultraDuploCreateProxy(spec, transform = null) {
  ...
  try {
    ...
    if (transform) {
      ...
      mesh.scale.set(
        spec.w * transform.scale.x,
        spec.h * transform.scale.y,
        spec.d * transform.scale.z,
      );
...
async function ultraDuploLoadGlb(spec, glbSrc) {
  ...
  if (size.lengthSq() < 1e-10) throw new Error('GLB enthält keine messbare Geometrie.');

  // Wrapper hat Identität; der Box-Mittelpunkt liegt damit in seinem Raum.
  // Erst zentrieren, dann den Wrapper auf den Duplo-Slot einpassen.
  raw.position.sub(centre);
  const fit = Math.min(
    spec.w / Math.max(size.x, .001),
    spec.h / Math.max(size.y, .001),
    spec.d / Math.max(size.z, .001),
  );
  wrapper.scale.setScalar(fit);
  markEditorObject(wrapper, 'assembly', `🔎 ${spec.label} · GLB`);
  return wrapper;
}
...
async function ultraDuploExpand(spec, proxy) {
  ...
  if (glbSrc) {
    try {
      assembly = await ultraDuploLoadGlb(spec, glbSrc);
    } catch (err) {
  ...
  assembly.scale.multiply(relativeScale);
  ...
  pushHistory();
  selectObject(null);
  objectsRoot.remove(proxy);
  disposeObject(proxy);
  objectsRoot.add(assembly);
  assembly.updateMatrixWorld(true);
  refreshSolidComponents();
  renderObjectList();
  selectObject(assembly);
  focusObject(assembly);
...
function ultraDuploCollapse(detail) {
  ...
  pushHistory();
  selectObject(null);
  objectsRoot.remove(detail);
  disposeObject(detail);
...
function ultraDuploSetSnap(m) {
  ...
  updateSnap();
  document.querySelectorAll('[data-ultra-snap]').forEach((b) =>
    b.classList.toggle('active', Number(b.dataset.ultraSnap) === m));
...
function ultraDuploInspect(id) {
  const object = ultraDuploFind(id);
  if (!object) return null;
  const d = dimensions(object);
  return {
    id,
    detail: !!object.userData?.duploDetailOf,
    position: object.position.toArray(),
    rotationY: object.rotation.y,
    dimensions: d.toArray(),
...
window.__BEUTELTIER_EDITOR__.state = () => {
  const pieces = objectsRoot.children.filter((o) => o.userData?.ultraDuplo || o.userData?.duploDetailOf).length;
  return {
    ...ultraDuploBaseState(),
    ultraDuploActive: pieces > 0,
    ultraDuploPieces: pieces,
