 ...
 * Baukloetze zusammengesteckt. Jeder Klotz kennt seinen semantischen Inhalt.
...
async function ultraDuploExpand(spec, proxy) {
  ...
  if (glbSrc) {
    try {
      ...
      assembly.position.set(-centre.x * fit, -centre.y * fit, -centre.z * fit);
    } catch (err) {
  ...
  const relativeScale = proxy.scale.clone();
  const deltaRotation = proxy.rotation.y - THREE.MathUtils.degToRad(spec.rotationY || 0);
  if (glbSrc) {
    const centredOffset = assembly.position.clone();
    assembly.position.copy(proxy.position).add(centredOffset.applyAxisAngle(new THREE.Vector3(0, 1, 0), deltaRotation));
  } else {
